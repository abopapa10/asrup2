...
/**
 * Server-rendered FAQ — `<details>`/`<summary>` üzerine kuruludur.
 * Client JS gerektirmez, SEO için ideal markup üretir.
 *
 * Görsel dil; `globals.css` içindeki `.faq-item` ve `.faq-answer`
 * yardımcı sınıflarıyla mevcut FaqAccordion ile uyumludur.
 */
export function SeoFaq({ heading = "Sıkça Sorulan Sorular", items }: SeoFaqProps) {
    ...
    <FadeIn as="section" className="max-w-3xl">
      ...
      <div className="mt-10 flex flex-col gap-3 sm:mt-12 sm:gap-4">
        {items.map((item, index) => (
          <details
            key={item.question}
            className="seo-faq-item faq-item group"
          >
